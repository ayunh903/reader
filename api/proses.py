...
async def proses(files: list[UploadFile] = File(...)):
    ...
    if not all_rows:
        return JSONResponse(content={"error": "Tidak ada data valid ditemukan"}, status_code=400)

    df = pd.DataFrame(all_rows)
    summary = df.groupby("Nama Fasilitas").agg({
        "Plafon": "count",
        "Baki Debet": "sum"
    }).rename(columns={"Plafon": "Jumlah Fasilitas", "Baki Debet": "Total Baki Debet"})

    df_output = df.to_dict(orient="records")
    summary_output = summary.reset_index().to_dict(orient="records")

    return {"data": df_output, "summary": summary_output}
